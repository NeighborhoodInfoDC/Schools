...
proc means data = minmax n max min;
	class ui_id;
	var 
		%do i = 01 %to 13; %let val=%sysfunc(putn(&i,z2.));
			aud_&val._n adult_flag_&val. 
		%end;;
	output out= minmax 
	min(%do i = 01 %to 13;%let val=%sysfunc(putn(&i,z2.));
			aud_&val._n 
		%end;)=%do i = 01 %to 13; %let val=%sysfunc(putn(&i,z2.));
			grade_min_20&val.
		%end; 
	max(%do i = 01 %to 13; %let val=%sysfunc(putn(&i,z2.));
			aud_&val._n  adult_flag_&val. 
		%end;)=%do i = 01 %to 13; %let val=%sysfunc(putn(&i,z2.));
...
data enroll.minmax_1314; set minmax_and_open; run;
