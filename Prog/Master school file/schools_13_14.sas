/**************************************************************************
 ...
 Description: Adds newest year (2013-2014)
 Modifications:
**************************************************************************/
...
libname nmast "K:\Metro\PTatian\DCData\Libraries\Schools\Data\Master school file\13_14_data";
libname omast "K:\Metro\PTatian\DCData\Libraries\Schools\Data\Master school file\Archive";
...
proc sort data=omast.msf0012 out=old_msf; by ui_id; run;
...
data all_years;
	...
	if compare(school_name_20&y1.,'')=0 then do;
		if compare(school_name_20&prevY.,'CLOSED')=0 then school_name_20&y1.="CLOSED";
	end;
	...
	if compare(school_name_20&y1.,'')=0 then do;
		...
		ui_id='2100802' OR
		ui_id='2100803' OR
		ui_id='2104800' ) then school_name_20&y1.="CLOSED";
	end;
run;
...
data closed_subset;
	set all_years;
	if compare(school_name_2013,'')=1 then delete;
run;
data new_schools;
	set all_years;
	if new_school=. then delete;
run;
*/
...
/*merge current year to old years*/
	data all_years_geo (drop=geoblk2000);
		merge %do y=20&newY1. %to 20&newY1.; 
			other_geocode_&y.
			%do i = 1 %to &words.; 
				%let word = %scan(&words.,&i.," ");
				%let dset = %scan(&dsets.,&i.," ");
...
data msf_enroll;
	merge nmast.msf00&newY1. enroll.enroll_to_merge_&newY1.&newY2.;
	by ui_id;
run;
...
data schools.MSF_sum&filesuf (label=&file_lbl);
  ...
  creator_process=schools_13_14,
  restrictions=None,
  revisions=%str(Updated with 2013-2014 SY data)
)
